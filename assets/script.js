...
// Works with mouse AND touch throughout.
  ...
  document.querySelectorAll('.utab').forEach(tab=>{
    tab.addEventListener('click', ()=>{
      ...
      // swap the reference illustration to match ม.ต้น / ม.ปลาย
      const mainImg = document.getElementById('uniformMainImg');
      if(mainImg && tab.dataset.img){
        mainImg.src = tab.dataset.img;
        mainImg.alt = tab.dataset.alt || mainImg.alt;
      }
    });
  });
